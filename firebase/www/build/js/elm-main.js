...
Elm.Tool = Elm.Tool || {};
Elm.Tool.make = function (_elm) {
   "use strict";
   _elm.Tool = _elm.Tool || {};
   if (_elm.Tool.values) return _elm.Tool.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var doA = 0;
   return _elm.Tool.values = {_op: _op,doA: doA};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var actionMailbox = $Signal.mailbox("none");
   var cmd = Elm.Native.Port.make(_elm).inboundSignal("cmd",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",v);
   });
   var input = $Signal.mergeMany(_U.list([actionMailbox.signal,cmd]));
   var model = Elm.Native.Port.make(_elm).outboundSignal("model",
   function (v) {
      return v;
   },
   A3($Signal.foldp,F2(function (cmd,model) {    return cmd;}),"init",input));
   return _elm.Main.values = {_op: _op,actionMailbox: actionMailbox,input: input};
};
