...
(defn update [ctx]
  (cond
    ...
    (let [idx (->>
                   ...
                   (ffirst))]
      (merge ctx {:mouseDown false :select1 idx :state :select2}))
    ...
    (let [idx (->>
                   ...
                   (ffirst))]
      (if-not idx
        (merge ctx {:mouseUp false :select1 nil :state :select1})
        (let [s1 (:select1 ctx)
              s2 idx
              pass (checkAnswer ctx s1 s2)]
          (if-not pass
            (merge ctx {:mouseUp false :select1 nil :state :select1})
            (merge ctx {:mouseUp false :select1 nil :buildLine (conj (:buildLine ctx) [s1 s2]) :state :check})))))
        ...
        (do
          ...
          (doall 
            (doseq [c (concat (:leftCircle model) (:rightCircle model))]
              (let [[x y radius] c
                    selected (isPointInCircle (:mousePos model) c)]
                (if-not selected
                  (do
                    (.fill pc 255)
          ...
          (doall
            (doseq [line (:buildLine model)]
              (let [[s1 s2] line
                ...
                (.line pc x y x2 y2))))
          (.fill pc 255)
          (.ellipse pc (nth (:mousePos model) 0) (nth (:mousePos model) 1) 10 10))))))
