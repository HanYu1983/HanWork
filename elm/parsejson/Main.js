...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var inputJson = Elm.Native.Port.make(_elm).inboundSignal("inputJson","Json.Decode.Value",function (v) {    return v;});
   var Wine = function (a) {
      return function (b) {
         return function (c) {
            return function (d) {
   ...
   var Food = F3(function (a,b,c) {    return {name: a,image: b,description: c};});
   var apply = $Json$Decode.object2(F2(function (x,y) {    return x(y);}));
   var constructing = $Json$Decode.succeed;
   var parseWineList = function () {
      var parseWine = function (err) {
         var _p0 = err;
         if (_p0.ctor === "Just") {
               ...
               A2(apply,
               A2(apply,
               A2(apply,constructing(Wine),A2($Json$Decode._op[":="],"name",$Json$Decode.string)),
               A2($Json$Decode._op[":="],
               "region",
   ...
   var index = Elm.Native.Task.make(_elm).performSignal("index",
   A2($Signal.map,
   function (n) {
      var _p1 = n;
      if (_p1.ctor === "Ok") {
            ...
            A2($Task.map,
            function (_p2) {
               return A2($Maybe.map,function (_p3) {    return A2($Debug.log,"name",function (_) {    return _.name;}(_p3));},$List.head(_p2));
            },
            $Task.succeed(_p1._0)));
         } else {
            return A2($Task.map,$Basics.always({ctor: "_Tuple0"}),A2($Task.map,$Debug.log("err"),$Task.succeed(_p1._0)));
         }
   },
   A2($Signal.map,$Json$Decode.decodeValue(parseWineList),inputJson)));
   return _elm.Main.values = {_op: _op,constructing: constructing,apply: apply,Food: Food,Wine: Wine,parseWineList: parseWineList};
};
