...
(function (Buffer){
var fs, context, parse, readable, x$;
fs = require('fs');
context = {
  state: 0,
  pos: 0,
  buf: new Buffer(65535 * 10)
};
console.log(context.buf.byteLength);
parse = function(ctx){
  var state, pos, buf, magic, dataLen, rows, columns, imagelen, i;
  ...
  return (function(){
    ...
    case 1:
      ...
      if (pos >= imagelen) {
        ...
        console.log("imagelen is " + imagelen);
...
readable = (x$ = fs.createReadStream("../doc/train-images-idx3-ubyte"), x$.on("data", function(trunk){
  ...
  for (;;) {
    ...
    if (ok) {
      ...
      results$.push(context.columns = nctx.columns);
    } else {
      break;
    }
  }
